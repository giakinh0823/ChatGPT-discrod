...
import discord
from app.base import Message, Prompt, Conversation
from app.utils import split_into_shorter_messages, close_thread, logger
    ...
    if status is CompletionResult.OK or status is CompletionResult.MODERATION_FLAGGED:
        ...
        if not reply_text:
            sent_message = await thread.send(
                embed=discord.Embed(
                    description=f"**Phản hồi không hợp lệ** - phản hồi trống",
        ...
        if status is CompletionResult.MODERATION_FLAGGED:
            await send_moderation_flagged_message(
                ...
                message=reply_text,
                url=sent_message.jump_url if sent_message else "không có url",
            )

            await thread.send(
                embed=discord.Embed(
                    description=f"⚠️ **Cuộc trò chuyện này đã được kiểm duyệt gắn cờ.**",
                    color=discord.Color.yellow(),
                )
            )
        ...
        await thread.send(
            embed=discord.Embed(
                description=f"❌ **Phản hồi đã bị quản trị viên chặn.**",
        ...
        await thread.send(
            embed=discord.Embed(
                description=f"**Yêu cầu không hợp lệ** - {status_text}",
                color=discord.Color.yellow(),
            )
        )
    else:
